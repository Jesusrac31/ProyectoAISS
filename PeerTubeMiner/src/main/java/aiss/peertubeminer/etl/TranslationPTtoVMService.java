...
public class TranslationPTtoVMService{
    ...
    UserVM resUserVM = new UserVM(accountPT.getId(), accountPT.getName(), accountPT.getUrl(), accountPT.getAvatars().get(0).getFileUrl()); // We use get(0) as an alternative to getFirst(). This allows the program to compile in more versions of Java
    return resUserVM;
    }
